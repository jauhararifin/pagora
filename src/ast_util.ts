...
function encodeAssignStatement (node: AssignStatementNode): any[] {
  return [encodeExprNode(node.receiver), node.assign.repr(), encodeExprNode(node.value)]
}

function encodeReturnStatement (node: ReturnStatementNode): any[] {
  return [node.return.repr(), (node.value != null) ? encodeExprNode(node.value) : undefined]
}
